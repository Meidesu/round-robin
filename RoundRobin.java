...
public class RoundRobin {
  public static void main(String[] args) {
    ...
    listaDeProcessos = listaDeProcessos.stream().sorted((p1, p2) -> p1.tempoIngresso.compareTo(p2.tempoIngresso))
        .toList();
    // ordena os tempos de ingresso para começar pelo processo com o menor tempo

    // Fazendo com que o tempo de ingresso dos processos sejam sempre relativos ao
    // menor tempo de ingresso
    // Isso quer dizer que se existem 2 processos A e B em que A entra no tempo 1 e
    // B no tempo 4, na verdade eles entram nos tempos 0 e 3
  ...
  private static ResultadosRoundRobin escalonadorRoundRobin(Queue<Processo> filaDeProcessos, int quantum,
      int trocaDeContexto) {
    ...
    while (!filaDeProcessos.isEmpty()) {
      ...
      int tempoExecutado = filaDeProcessos.size() > 0 ? Math.min(quantum, processoAtual.tempoProcesso)
          : processoAtual.tempoProcesso; // Tempo que o processo executa na volta
      System.out.println("Executando " + processoAtual.nome + " por " + tempoExecutado + " unidades de tempo : total: "
          + processoAtual.tempoProcesso);
      ...
      if (processoAtual.estaAtivo() && tempoExecutado > 0) {
        ...
        filaDeProcessos.add(processoAtual);
        System.out
            .println(processoAtual.nome + " ainda tem trabalho a ser feito. Adicionando de volta à fila. -> Restante: "
                + processoAtual.tempoProcesso);
      } else {
      ...
      // Verifico os processos que estão ativos e adciono a todos a troca de contexto
      // no tempo de espera
